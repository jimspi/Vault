...
export async function POST(request: NextRequest) {
  try {
    ...
    const { workspaceId, userId, isAutoGenerated } = body;
    ...
    if (isServiceCall && userId) {
      // Service role call - use provided userId
      ownerId = userId;
      console.log('[Recommendations] Service role call for user:', userId);
    } else {
      ...
      try {
        const { data: saved, error: insertError } = await supabase
          ...
          .insert({
            ...
            status: 'new',
            metadata: {
              auto_generated: isAutoGenerated || false,
